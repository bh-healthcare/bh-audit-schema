...
- **HL7 `auditevent-OnBehalfOf` extension** (FHIR Core Extensions Registry). Expresses a single delegation hop on `AuditEvent.agent`. The community shipping this extension is direct evidence that delegation is a recognized gap in base AuditEvent. The profile is compatible with `auditevent-OnBehalfOf` but does not rely on it alone: that extension collapses the authenticating-vs-authorizing distinction, which is exactly the split that matters in the agent-under-its-own-credential case (`actor.subject_type == "agent"` in the source schema). The three-slice design is what makes that split queryable.
